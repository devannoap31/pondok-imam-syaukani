  ...
  <section class="admin-content-section active block">
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8 mb-8">
      ...
      <div class="flex justify-between items-center gap-3.5 mb-6 flex-wrap">
        <h3 class="text-lg font-bold font-outfit text-primary">Jadwal Harian Santri</h3>
        <a href="{{ route('jadwal.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-primary text-white rounded-full text-xs font-semibold hover:bg-primary-dark transition-all">+ Tambah Jadwal</a>
      </div>
        ...
        <table class="w-full border-collapse bg-white text-left text-xs sm:text-sm">
          ...
          <tbody class="divide-y divide-slate-100">
            ...
            <tr class="even:bg-slate-50 hover:bg-slate-50/50 transition-colors">
              ...
              <td class="p-4 font-semibold text-slate-700">{{ $item->waktu }}</td>
            ...
            @empty
            <tr>
              <td colspan="4" class="p-4 text-center text-slate-500">Belum ada jadwal.</td>
            </tr>
            @endforelse
          </tbody>
        </table>
