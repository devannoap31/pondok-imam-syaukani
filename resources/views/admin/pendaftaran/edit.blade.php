    ...
    <div class="bg-white rounded-2xl sm:rounded-3xl border border-slate-200 shadow-xs p-6 sm:p-8 flex-1 flex flex-col justify-between w-full">
      <div>
        ...
        <form id="pendaftaranEditForm" action="{{ route('pendaftaran.update', $pendaftar) }}" method="POST" class="space-y-6">
          ...
          <div class="mt-8 border-t border-slate-200 pt-6">
              ...
              @if(count($pendaftar->berkas) > 0)
                  <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div class="border border-slate-200 rounded-xl p-4 flex flex-col items-center justify-center gap-2 hover:border-primary/40 hover:bg-slate-50/50 transition-all">
                          <span class="text-xs font-bold text-slate-500 uppercase tracking-wide">{{ $berkas->jenis_berkas }}</span>
                          <a href="{{ Storage::url($berkas->file_path) }}" target="_blank" class="text-primary hover:underline text-sm font-semibold flex flex-col items-center gap-1 group">
                              <span class="text-3xl transition-transform duration-200 group-hover:scale-110">📄</span>
          ...
          <div class="pt-6">
              <button type="submit" class="inline-flex items-center justify-center px-8 py-3 bg-primary text-white rounded-xl text-sm font-semibold hover:bg-primary-dark active:scale-[0.98] transition-all shadow-sm cursor-pointer">
