...
@section('title', 'Catat Donasi Manual – Dashboard Admin')
  ...
  <section class="admin-content-section active block">
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8 mb-8 max-w-4xl">
      ...
      <div class="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
        <div>
          <h3 class="text-lg sm:text-xl font-bold font-outfit text-slate-800">Catat Donasi Manual</h3>
          <p class="text-xs text-slate-500 mt-0.5">Input transaksi donasi langsung / offline ke dalam sistem.</p>
        </div>
      ...
      @php
        $autoIdTransaksi = (int) (date('ymd') . rand(1000, 9999));
      @endphp
      ...
      <form action="{{ route('donasi.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <!-- ID Transaksi (Auto-generated by default) -->
          <div>
            ...
            <input 
              ...
              required 
              value="{{ old('id_transaksi', $autoIdTransaksi) }}" />
          </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input 
              ...
              class="w-full px-4.5 py-3 border border-slate-300 rounded-xl text-sm transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none bg-white font-medium" 
              value="{{ old('metode_pembayaran', 'Transfer Bank BSI') }}" />
          </div>
        </div>
        ...
        <!-- Bukti Pembayaran / Struk (Opsional untuk manual) -->
        <div>
          <label class="block text-slate-700 text-xs font-bold uppercase tracking-wider mb-2">
            Lampirkan Bukti Transfer / Struk (Opsional)
          </label>
          <input 
            type="file" 
          ...
          <p class="text-[11px] text-slate-400 mt-1">Format: JPG, PNG, atau PDF (Maks. 5MB)</p>
        </div>
