...
@section('title', 'Tambah Fasilitas & Data Statistik – Dashboard Admin')
  ...
  <div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8 mb-8">
      ...
      <div class="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
        ...
        <div>
          <h3 class="text-lg font-bold font-outfit text-primary">Tambah Fasilitas / Data Statistik</h3>
          <p class="text-xs text-slate-400 mt-0.5">Tambahkan baris data baru untuk tabel statistik pondok</p>
        </div>
      </div>

      <form action="{{ route('fasilitas-admin.store') }}" method="POST" class="space-y-5">
        @csrf
        ...
        <div class="grid grid-cols-1 sm:grid-cols-[1fr_120px] gap-4">
          <div>
            ...
            <input type="text" id="deskripsi_singkat" name="deskripsi_singkat" value="{{ old('deskripsi_singkat') }}"
        ...
        <div>
          ...
          <textarea id="detail" name="detail" rows="4"
                    ...
                    class="w-full px-4 py-2.5 text-sm rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all resize-none @error('detail') border-red-400 @enderror">{{ old('detail') }}</textarea>
            ...
            <label class="flex items-center gap-3 mt-2.5 cursor-pointer group">
              <div class="relative">
                <input type="checkbox" id="aktif" name="aktif" value="1"
                       {{ old('aktif', '1') ? 'checked' : '' }} class="sr-only peer"/>
