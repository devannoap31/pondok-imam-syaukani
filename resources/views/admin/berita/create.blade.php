    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8 mb-8">
      ...
      <form action="{{ route('berita.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div class="form-group">
          ...
          <input type="text" name="kategori" class="w-full px-4.5 py-3 border border-slate-300 rounded-xl text-sm transition-all focus:border-primary focus:shadow-[0_0_0_3px_rgba(18,78,63,0.1)] focus:outline-none bg-white" required value="{{ old('kategori') }}" placeholder="Contoh: Pengumuman, Prestasi, Artikel" />
        </div>
        <div class="form-group">
