    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <div class="flex items-center gap-2 text-xs font-semibold text-slate-400 mb-1">
          ...
          <span class="text-primary font-bold">Edit Berita</span>
        </div>
        <h3 class="text-xl sm:text-2xl font-bold font-outfit text-slate-800">Edit Berita</h3>
        <p class="text-xs text-slate-500 mt-0.5">Perbarui informasi, konten, atau gambar berita yang sudah dipublikasikan.</p>
      </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div class="md:col-span-2 space-y-5">
            ...
            <div class="form-group">
              ...
              <input type="text" name="judul" class="w-full px-4 py-3 border border-slate-300 rounded-xl text-sm transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none bg-white font-medium text-slate-800" required value="{{ old('judul', $berita->judul) }}" placeholder="Masukkan judul berita utama..." />
            </div>

            <!-- Isi Berita -->
            <div class="form-group">
              <label class="block text-slate-700 text-xs font-bold mb-2">Isi Konten Berita <span class="text-rose-500">*</span></label>
              <textarea name="isi" class="w-full px-4 py-3 border border-slate-300 rounded-xl text-sm transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none resize-y min-h-[250px] bg-white leading-relaxed text-slate-800" required placeholder="Tuliskan isi berita secara mendalam di sini...">{{ old('isi', $berita->isi) }}</textarea>
            </div>
          </div>
          ...
          <div class="space-y-5">
            ...
            <div class="form-group">
              ...
              <input type="text" id="kategoriInput" name="kategori" class="w-full px-4 py-3 border border-slate-300 rounded-xl text-sm transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none bg-white font-medium text-slate-800 mb-2" required value="{{ old('kategori', $berita->kategori) }}" placeholder="Contoh: Pengumuman, Prestasi, Artikel" />
            ...
            <div class="form-group">
              ...
              @php
                $formattedDate = old('tanggal_publish', $berita->tanggal_publish ? \Carbon\Carbon::parse($berita->tanggal_publish)->format('Y-m-d') : date('Y-m-d'));
              @endphp
              <input type="date" name="tanggal_publish" class="w-full px-4 py-3 border border-slate-300 rounded-xl text-sm transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none bg-white font-medium text-slate-800" required value="{{ $formattedDate }}" />
            </div>

            <!-- Upload Gambar -->
            <div class="form-group">
              <label class="block text-slate-700 text-xs font-bold mb-2">Gambar Berita</label>
              ...
              <div class="relative border-2 border-dashed border-slate-300 hover:border-primary/50 bg-slate-50/50 rounded-2xl p-3 text-center transition-all group">
                <input type="file" name="gambar" id="gambarInput" accept="image/jpeg,image/png,image/jpg,image/webp" class="hidden" onchange="previewImage(event)" />
                
                <div id="imagePreviewContainer" class="relative">
                  @if($berita->gambar)
                    <div class="relative rounded-xl overflow-hidden shadow-xs border border-slate-200 group">
                      <img id="imagePreview" src="{{ asset('storage/' . $berita->gambar) }}" alt="Preview Gambar Berita" class="w-full h-44 object-cover rounded-xl" />
                      <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                        <label for="gambarInput" class="px-3 py-1.5 bg-white text-slate-800 text-xs font-bold rounded-lg cursor-pointer hover:bg-slate-100 shadow-sm transition-transform hover:scale-105">Ganti Gambar</label>
                      </div>
                    </div>
                  @else
                    <img id="imagePreview" src="" alt="Preview Gambar" class="hidden w-full h-44 object-cover rounded-xl mb-2" />
              ...
              <p class="text-[11px] text-slate-400 mt-1 italic">*Biarkan kosong jika tidak ingin mengubah gambar saat ini.</p>
            </div>
          </div>
        </div>
