...
@section('title', 'Edit Penyaluran Dana – Dashboard Admin')
  ...
  <section class="admin-content-section active block">
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8 mb-8 max-w-4xl">
      ...
      <div class="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
        <div>
          <h3 class="text-lg sm:text-xl font-bold font-outfit text-slate-800">Edit Card Penyaluran Dana</h3>
          <p class="text-xs text-slate-500 mt-0.5">Perbarui informasi program penyaluran donasi.</p>
        </div>
      ...
      <form action="{{ route('penyaluran-dana.update', $penyaluran->id_penyaluran) }}" method="POST" class="space-y-6" x-data="{
        selectedIcon: '{{ old('icon', $penyaluran->icon) }}',
        selectedColor: '{{ old('warna', $penyaluran->warna) }}'
      }">
        @csrf
        @method('PUT')
        ...
        <div>
          ...
          <input 
            ...
            required 
            value="{{ old('judul', $penyaluran->judul) }}" />
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input 
              ...
              required 
              value="{{ old('urutan', $penyaluran->urutan) }}" />
          </div>
        </div>
        ...
        <div>
          ...
          <input type="hidden" id="iconInput" name="icon" value="{{ old('icon', $penyaluran->icon) }}" :value="selectedIcon">
          ...
          <p class="text-[11px] text-slate-500">Icon terpilih saat ini: <strong class="text-primary font-mono" x-text="selectedIcon">{{ old('icon', $penyaluran->icon) }}</strong></p>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div>
            ...
            <select name="warna" x-model="selectedColor" class="w-full px-4.5 py-3 border border-slate-300 rounded-xl text-sm transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none bg-white font-medium">
              <option value="emerald" {{ old('warna', $penyaluran->warna) == 'emerald' ? 'selected' : '' }}>Emerald (Hijau Islami - Utama)</option>
              <option value="amber" {{ old('warna', $penyaluran->warna) == 'amber' ? 'selected' : '' }}>Amber (Kuning Keemasan)</option>
              <option value="teal" {{ old('warna', $penyaluran->warna) == 'teal' ? 'selected' : '' }}>Teal (Toska / Fasilitas)</option>
              <option value="blue" {{ old('warna', $penyaluran->warna) == 'blue' ? 'selected' : '' }}>Blue (Biru Profesional / Guru)</option>
            </select>
          </div>
          <div>
            <label class="block text-slate-700 text-xs font-bold uppercase tracking-wider mb-2">
            ...
            <select name="aktif" class="w-full px-4.5 py-3 border border-slate-300 rounded-xl text-sm transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none bg-white font-medium">
              <option value="1" {{ old('aktif', $penyaluran->aktif ? '1' : '0') == '1' ? 'selected' : '' }}>Aktif (Ditampilkan)</option>
              <option value="0" {{ old('aktif', $penyaluran->aktif ? '1' : '0') == '0' ? 'selected' : '' }}>Nonaktif (Disembunyikan)</option>
            </select>
          </div>
        </div>
        ...
        <div>
          ...
          <textarea 
            ...
            class="w-full px-4.5 py-3 border border-slate-300 rounded-xl text-sm transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none bg-white" 
            required>{{ old('deskripsi', $penyaluran->deskripsi) }}</textarea>
        </div>
