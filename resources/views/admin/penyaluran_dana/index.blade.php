...
@section('title', 'Kelola Penyaluran Dana – Dashboard Admin')
  ...
  <section class="admin-content-section active block">
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8 mb-8">
      ...
      <div class="flex justify-between items-center gap-4 mb-6 flex-wrap">
        <div>
          <h3 class="text-lg sm:text-xl font-bold font-outfit text-slate-800">Daftar Card Penyaluran Dana</h3>
          <p class="text-xs text-slate-500 mt-0.5">Program penggunaan dan alokasi dana donasi yang tampil di halaman depan.</p>
        </div>
        <div class="flex items-center gap-2.5">
          ...
          <a href="{{ route('penyaluran-dana.create') }}" class="inline-flex items-center justify-center gap-1.5 px-4 py-2 bg-primary text-white rounded-xl text-xs font-bold hover:bg-primary-dark transition-all shadow-sm shadow-primary/20">
        ...
        <table class="w-full border-collapse bg-white text-left text-xs sm:text-sm">
          <thead>
            <tr class="bg-primary text-white">
              ...
              <th class="p-4 font-semibold text-xs uppercase tracking-wider w-20 text-center">Icon</th>
              <th class="p-4 font-semibold text-xs uppercase tracking-wider">Judul & Tag</th>
          ...
          <tbody class="divide-y divide-slate-100">
            ...
            <tr class="even:bg-slate-50/70 hover:bg-slate-50 transition-colors">
              ...
              <td class="p-4 text-center">
                <div class="w-10 h-10 mx-auto rounded-xl bg-emerald-50 text-primary border border-emerald-200/70 flex items-center justify-center">
                  <x-donasi-icon :name="$item->icon" class="w-5 h-5 text-primary" />
                </div>
              </td>
              <td class="p-4">
                <div class="font-bold font-outfit text-slate-800 text-sm">{{ $item->judul }}</div>
                @if($item->tag)
                  <span class="inline-block mt-1 px-2.5 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider bg-emerald-100 text-emerald-800">
                    {{ $item->tag }}
                  </span>
                @endif
              </td>
              ...
              <td class="p-4 text-center space-x-1.5">
                <a href="{{ route('penyaluran-dana.edit', $item->id_penyaluran) }}" class="inline-flex items-center justify-center px-3 py-1.5 bg-slate-100 text-slate-700 hover:bg-primary hover:text-white rounded-lg text-xs font-bold transition-all">Edit</a>
                <form action="{{ route('penyaluran-dana.destroy', $item->id_penyaluran) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data penyaluran ini?');">
                  @csrf
                  @method('DELETE')
            ...
            @empty
            <tr>
              <td colspan="6" class="p-8 text-center text-slate-400 font-medium">Belum ada data penyaluran dana. Klik "Tambah Card Program" untuk membuat baru.</td>
            </tr>
            @endforelse
          </tbody>
        </table>
