...
@section('title', 'Tambah Tujuan Donasi – Dashboard Admin')
  ...
  <section class="admin-content-section active block">
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8 mb-8 max-w-4xl">
      ...
      <div class="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
        <div>
          <h3 class="text-lg sm:text-xl font-bold font-outfit text-slate-800">Tambah Card Tujuan Donasi</h3>
          <p class="text-xs text-slate-500 mt-0.5">Tambahkan tujuan, visi, atau dampak kebaikan donasi ke halaman depan.</p>
        </div>
      ...
      <form action="{{ route('tujuan-donasi.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div>
          ...
          <select name="warna" class="w-full px-4.5 py-3 border border-slate-300 rounded-xl text-sm transition-all focus:border-primary focus:ring-2 focus:ring-primary/20 focus:outline-none bg-white font-medium">
            <option value="primary" {{ old('warna') == 'primary' ? 'selected' : '' }}>Hijau Emerald (Primary)</option>
            <option value="accent" {{ old('warna') == 'accent' ? 'selected' : '' }}>Kuning Emas (Accent)</option>
            <option value="primary-light" {{ old('warna') == 'primary-light' ? 'selected' : '' }}>Toska Muda (Primary Light)</option>
          </select>
        </div>
