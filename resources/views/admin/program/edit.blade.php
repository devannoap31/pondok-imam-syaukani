...
@section('content')
  <section class="admin-content-section active block">
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8 mb-8">
      ...
      <form action="{{ route('program-pendidikan.update', $program) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
          ...
          <input type="text" name="subjudul" class="w-full px-4.5 py-3 border border-slate-300 rounded-xl text-sm transition-all focus:border-primary focus:outline-none bg-white" value="{{ old('subjudul', $program->subjudul) }}" placeholder="Contoh: Madrasah Tsanawiyah" />
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <textarea name="keunggulan_text" class="w-full px-4.5 py-3 border border-slate-300 rounded-xl text-sm transition-all focus:border-primary focus:outline-none resize-y min-h-[120px] bg-white" placeholder="Satu keunggulan per baris">{{ old('keunggulan_text', implode("\n", $program->keunggulan ?? [])) }}</textarea>
        </div>
        <div class="grid sm:grid-cols-2 gap-5">
          <div class="form-group">
            <label class="block text-slate-700 text-xs font-bold mb-2">Urutan Tampil</label>
            <input type="number" name="urutan" min="0" class="w-full px-4.5 py-3 border border-slate-300 rounded-xl text-sm focus:border-primary focus:outline-none bg-white" value="{{ old('urutan', $program->urutan) }}" />
          </div>
          <label class="flex items-center gap-3 text-sm text-slate-700 font-semibold sm:pt-8">
            <input type="checkbox" name="aktif" value="1" class="w-4 h-4 accent-primary" {{ old('aktif', $program->aktif) ? 'checked' : '' }} />
