...
@section('content')
  ...
  <section class="py-16 sm:py-20 bg-slate-50" x-data="{
    ...
    <div class="max-w-[1200px] mx-auto px-6">
      ...
      <!-- SECTION 3: SALURKAN DONASI (TOMBOL & METODE PEMBAYARAN) -->
      <div id="metode-donasi" class="scroll-mt-24">
        <div class="text-center max-w-2xl mx-auto mb-12">
          <span class="inline-block bg-accent/20 text-accent-dark text-xs font-bold uppercase tracking-[1.5px] px-4 py-1.5 rounded-full mb-3">
            Salurkan Kebaikan
          </span>
          <h2 class="text-2xl sm:text-3xl md:text-4xl font-extrabold font-outfit text-slate-800">
            Pilihan Metode Penyaluran Donasi
          </h2>
          <p class="text-slate-600 text-sm sm:text-base mt-2.5">
            Pilih cara yang paling mudah bagi Anda untuk menyalurkan infaq, sedekah, maupun wakaf.
          </p>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
          
          <!-- CARD 1: TRANSFER BANK BSI -->
          <div class="bg-white rounded-3xl p-7 sm:p-9 border border-slate-200/80 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between" x-data="{ copied: false }">
            <div>
              <div class="flex items-center justify-between gap-4 mb-6">
                <span class="inline-block bg-emerald-50 text-primary text-xs font-bold uppercase px-3 py-1.5 rounded-lg">
                  Transfer Bank
                </span>
                <span class="text-xs font-semibold text-slate-400">Verifikasi Manual</span>
              </div>

              <h3 class="text-xl font-bold font-outfit text-slate-800 mb-1">
              ...
              <!-- Box Nomor Rekening -->
              <div class="p-5 bg-slate-50 border border-slate-200/70 rounded-2xl mb-6">
                <span class="text-[11px] uppercase font-bold tracking-wider text-slate-400 block mb-1">Nomor Rekening Resmi</span>
                <div class="flex items-center justify-between gap-3">
                  <span id="nomorRekening" class="font-outfit text-xl sm:text-2xl font-extrabold text-primary tracking-wide">
                    7174567890
                  </span>
                  <!-- Tombol Salin -->
                  <button 
                    @click="navigator.clipboard.writeText('7174567890'); copied = true; setTimeout(() => copied = false, 2500)"
                    type="button"
                    class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold transition-all duration-200 shadow-sm"
                    :class="copied ? 'bg-emerald-600 text-white' : 'bg-primary text-white hover:bg-primary-dark active:scale-95'">
                ...
                <div class="mt-3 text-xs text-slate-600">
                  Atas Nama: <strong class="text-slate-800">PPTQ Imam Syaukani</strong>
                </div>
              </div>
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
              <span class="text-xs text-slate-500">Sudah melakukan transfer?</span>
              <a href="https://wa.me/{{ $waNum }}?text=Assalamu%27alaikum%20Panitia%20Donasi%20PPTQ%20Imam%20Syaukani%2C%20saya%20sudah%20transfer%20donasi%20melalui%20BSI..." target="_blank" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 text-white font-bold text-xs hover:bg-emerald-700 transition-colors shadow-sm">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <!-- CARD 2: SCAN QRIS INSTAN -->
          <div class="bg-white rounded-3xl p-7 sm:p-9 border border-slate-200/80 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between">
            <div>
              <div class="flex items-center justify-between gap-4 mb-6">
                <span class="inline-block bg-amber-50 text-amber-700 text-xs font-bold uppercase px-3 py-1.5 rounded-lg">
                  QRIS Semua Pembayaran
                </span>
                <span class="text-xs font-semibold text-emerald-600">Otomatis & Cepat</span>
              </div>
              ...
              <p class="text-xs sm:text-sm text-slate-500 mb-6">
                Mendukung Gopay, OVO, DANA, ShopeePay, BCA, Livin, BRImo, dll.
              </p>

              <!-- QR Code Display Area -->
              <div class="p-6 bg-slate-50 border border-slate-200/70 rounded-2xl flex flex-col items-center text-center mb-6">
                @if($qris && $qris->gambar_qris)
                  <img src="{{ asset('storage/' . $qris->gambar_qris) }}" alt="QRIS PPTQ Imam Syaukani" class="w-44 h-44 object-contain rounded-xl shadow-sm border border-slate-200 mb-3 bg-white p-2" />
                @else
                  <div class="w-44 h-44 bg-white rounded-xl shadow-sm border border-slate-200 mb-3 p-3 flex flex-col items-center justify-center">
                    <div class="w-36 h-36 bg-[repeating-conic-gradient(from_45deg,#144D30_0%_25%,#fff_0%_50%)] rounded-lg border-2 border-primary/20"></div>
                  </div>
                @endif
                <span class="text-xs font-bold text-slate-800">
                  @if($qris && $qris->nama_penerima) {{ $qris->nama_penerima }} @else PPTQ IMAM SYAUKANI @endif
                </span>
                <span class="text-[11px] text-slate-400 mt-0.5">NMID: ID1020304050607</span>
              </div>
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-center gap-3">
              @if($qris && $qris->gambar_qris)
                <a href="{{ asset('storage/' . $qris->gambar_qris) }}" download="QRIS_PPTQ_Imam_Syaukani.jpg" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-primary text-white font-bold text-xs hover:bg-primary-dark transition-colors shadow-sm">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              @else
                <a href="https://wa.me/{{ $waNum }}?text=Assalamu%27alaikum%20Panitia%20Donasi%20PPTQ%20Imam%20Syaukani%2C%20saya%20ingin%20meminta%20QRIS%20resmi%20pondok..." target="_blank" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-primary text-white font-bold text-xs hover:bg-primary-dark transition-colors shadow-sm">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" />
                  </svg>
                  Minta Kode QRIS via WA
                </a>
              @endif
            </div>
          </div>
