  ...
  <div class="w-full max-w-[450px] bg-white rounded-3xl shadow-xl p-8 sm:p-10 relative">
    ...
    <form onsubmit="handleLogin(event)" class="space-y-5">
      <div class="form-group">
        <label for="username" class="block text-slate-700 text-xs font-bold mb-2">Username / Email</label>
        <input type="text" id="username" class="w-full px-4.5 py-3 border border-slate-300 rounded-xl text-sm transition-all focus:border-primary focus:shadow-[0_0_0_3px_rgba(18,78,63,0.1)] focus:outline-none" placeholder="Masukkan Username atau email..." required />
      </div>
      
      <div class="form-group">
        <label for="password" class="block text-slate-700 text-xs font-bold mb-2">Password</label>
        <input type="password" id="password" class="w-full px-4.5 py-3 border border-slate-300 rounded-xl text-sm transition-all focus:border-primary focus:shadow-[0_0_0_3px_rgba(18,78,63,0.1)] focus:outline-none" placeholder="Masukkan Password..." required />
      </div>
      ...
      <div class="flex justify-between items-center gap-3.5 mb-6 text-xs sm:text-sm">
        <label class="flex items-center gap-1.5 cursor-pointer text-slate-500 font-medium select-none">
          <input type="checkbox" class="cursor-pointer" /> Ingat saya di Perangkat ini
        </label>
        <a href="#" class="text-primary font-bold hover:text-primary-dark transition-colors">Lupa Password?</a>
      </div>
    ...
    <div class="text-center mt-6 text-xs sm:text-sm">
      ...
      <a href="daftar.blade.php" class="text-primary font-bold hover:text-primary-dark transition-colors">Daftar Sekarang</a>
    </div>
    
    <div class="text-center mt-5">
      <a href="index.blade.php" class="text-xs text-slate-400 hover:text-primary transition-colors underline">Kembali ke Beranda</a>
    </div>
  </div>
  ...
  <script>
    function handleLogin(event) {
      ...
      if (user === 'admin' && pass === 'admin') {
        alert('Selamat Datang Super Admin!');
        window.location.href = 'dashboard.blade.php';
      } else {
        alert('Untuk demo, silakan masuk dengan username: admin dan password: admin');
      }
    }
  </script>
</body>
